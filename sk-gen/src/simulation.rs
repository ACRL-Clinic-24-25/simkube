...
//! 3. Graph contraction - Use contraction hierarchies to shrink the graph and create shortcuts to preserve the most probable paths
    ...
    F: Fn(&Node) -> Vec<Action> + Clone + Sync,
{
    ...
    // Use first event of first trace as the common starting state (may be None if all have been contracted).
    ...
    generate_traces(&processed_graph, &output_dir, num_samples, trace_length, first_trace_event)?;
    Ok(())
}

///  Phase 1 – Construct graph from observed traces
...
///  Phase 2 – Expand graph via enumerated synthetic actions (breadth-first)
    ...
    F: Fn(&Node) -> Vec<Action> + Clone + Sync,
{
    ...
    while !current_layer.is_empty() && depth < enumeration_steps {
        ...
        let to_process: Vec<_> = current_copy
            .into_iter()
...
///  Phase 3 – Graph contraction (Contraction Hierarchies)
...
fn contract_graph(
    ...
    let contraction_order = crate::contraction_hierarchies::nested_dissection_contraction_order(state_graph.clone());
...
///  Phase 4 – Trace generation via weighted random walks
#[instrument(skip(graph, initial_event))]
fn generate_traces(
    ...
    walks.par_iter().enumerate().try_for_each(|(i, walk)| -> Result<()> {
        ...
        generate_trace_file(graph, walk, &path, initial_event.clone())?;
        Ok(())
    })?;
    pb.finish_using_style();
